"""Masked AutoRL-SOP using Gaussian Process optimization with Hyperband pruning."""
...
TRAIN_PLOTS_PATH = os.path.join(SCRIPT_DIR, "Train Plots", "GP")
...
class SOPEnv:
    ...
    def render(self, window=100):
        ...
        plt.title(f'Final HYPERBAND GP Masked AutoRL-SOP Training - {self.problem_name}', fontsize=18)
...
if __name__ == "__main__":
    ...
    print(f"\nStarting GP + Hyperband Optimization (Single Seed: {SEED_VALUE})...")
    print(f"Stopping conditions: Target {TARGET_SOLUTION} OR Timeout {OPTUNA_TIMEOUT}s")
            
    os.makedirs(RESULTS_PATH, exist_ok=True)
    db_name = os.path.join(RESULTS_PATH, f"optuna_results_GP_{key_name}.db")
    db_url = f"sqlite:///{db_name}"
    sampler = optuna.samplers.GPSampler(seed=SEED_VALUE)
