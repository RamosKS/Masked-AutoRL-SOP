"""Masked AutoRL-SOP using multivariate TPE with Hyperband pruning."""
...
# Hyperband report interval (episodes between learning-curve checkpoints)
REPORT_INTERVAL = 500
...
TRAIN_PLOTS_PATH = os.path.join(SCRIPT_DIR, "Train Plots", "TPE")
...
class SOPEnv:
    ...
    def render(self, window=100):
        ...
        plt.title(f'Final HYPERBAND TPE Masked AutoRL-SOP Training - {self.problem_name}', fontsize=18)
...
def objective(trial, matrix_dist, precedences, target_val):
    ...
    # Training loop with intermediate reporting for Hyperband (multi-fidelity)
    for ep in range(env.n_training):
        env.reset()
        done = False
        current_action = None
        ...
        # Report the running-best distance (the learning curve) to the Hyperband
        # pruner and stop this configuration early if it is not promising.
        if (ep + 1) % REPORT_INTERVAL == 0:
            trial.report(best_cost_found_in_trial, ep + 1)
...
if __name__ == "__main__":
    ...
    print(f"\nStarting TPE + Hyperband Optimization (Single Seed: {SEED_VALUE})...")
    print(f"Stopping conditions: Target {TARGET_SOLUTION} OR Timeout {OPTUNA_TIMEOUT}s")
            
    os.makedirs(RESULTS_PATH, exist_ok=True)
    db_name = os.path.join(RESULTS_PATH, f"optuna_results_TPE_{key_name}.db")
    db_url = f"sqlite:///{db_name}"
    sampler = optuna.samplers.TPESampler(seed=SEED_VALUE, multivariate=True)
    pruner = optuna.pruners.HyperbandPruner(min_resource=REPORT_INTERVAL,
                                            max_resource=N_TRAIN_EPISODES,
                                            reduction_factor=3)
    study = optuna.create_study(direction='minimize', sampler=sampler, pruner=pruner, storage=db_url, study_name=f"study_{key_name}", load_if_exists=True)
