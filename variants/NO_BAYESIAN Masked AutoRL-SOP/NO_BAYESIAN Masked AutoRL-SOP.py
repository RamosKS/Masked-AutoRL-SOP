"""Masked AutoRL-SOP using sequential HyperTuningSK optimization."""
...
class SOPEnv:
    ...
    def render(self, window=100):
        ...
        plt.title(f'Final NO_BAYESIAN Masked AutoRL-SOP (HyperTuningSK) Training - {self.problem_name}', fontsize=18)
...
def run_training(matrix_dist, precedences, epsilon, alpha, gamma, n_episodes,
...
# (the best Scott-Knott group, i.e. lowest mean distance). If the model is not 
# adequate (non-normal residuals) or there is no significant difference, a single 
# random value in [min, max] is returned.
...
def run_phase(phase_name, phase_tag, tuned_values, fixed_eps, fixed_alpha, fixed_gamma,
              tuned_kind, matrix_dist, precedences, target_solution, tracker, log):
    ...
    for val in tuned_values:
        ...
        for epoch_i in range(EPOCHS):
            ...
            cost, best_ep, _ = run_training(matrix_dist, precedences, eps, alp, gam,
...
if __name__ == "__main__":
    ...
    esk = run_phase("Phase 1: tuning EPSILON (e-greedy)", "HPO_epsilon", HP_VALUES,
                    ...
                    GLOBAL_MATRIX_DIST, GLOBAL_PRECEDENCES, TARGET_SOLUTION, tracker, optimizer_log)
    ...
    for (e_val, a_val, g_val) in final_combos:
        ...
        best_cost, best_ep, env = run_training(
            GLOBAL_MATRIX_DIST, GLOBAL_PRECEDENCES,
