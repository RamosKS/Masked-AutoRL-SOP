"""Multivariate TPE ablation using resampling instead of action masking."""
...
class SOPEnv:
    ...
    def render(self, window=100):
        ...
        plt.title(f'Final NO_MASK Masked AutoRL-SOP (Resampling) Training - {self.problem_name}', fontsize=18)
...
def objective(trial, matrix_dist, target_val):
...
if __name__ == "__main__":
    ...
    # Ensure the Results folder (for the Optuna .db files) exists
    ...
    print(f"\nStarting TPE Optimization (Single Seed: {SEED_VALUE})...")
    ...
    objective_wrapper = lambda trial: objective(trial, GLOBAL_MATRIX_DIST, TARGET_SOLUTION)
